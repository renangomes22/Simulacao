...
# =====================================================================
st.set_page_config(
    page_title="GCP-Molas | Gêmeo Digital, APS & DDMRP V27",
    page_icon="⚙️",
    layout="wide",
...
st.sidebar.markdown("<div style='font-size: 20px; font-weight: bold; color: #1E3A8A; margin-bottom:15px;'>Painel de Configurações</div>", unsafe_allow_html=True)
...
with st.sidebar.expander("📝 4. Roteiros e Tempos (Produtividade)", expanded=False):
    ...
    if arquivo_roteiro is not None:
        try:
...
with st.sidebar.expander("📤 6. Upload de Planilha de Demanda", expanded=False):
    ...
    if arquivo_demanda is not None:
        try:
...
st.markdown("<div class='main-title'>GCP-Molas - Gêmeo Digital, APS & DDMRP V32</div>", unsafe_allow_html=True)
...
tab_painel, tab_gantt, tab_stochastic, tab_ddmrp, tab_demandas_ativas, tab_roteiros_ativos, tab_horas_extras = st.tabs([
    "📊 Resultados da Simulação",
    "📅 Cronograma de Gantt Interativo",
    "🧠 Análise de Riscos (Gêmeo Digital)",
    "🛑 Gestão de Buffers DDMRP",
    "📝 Demandas Cadastradas (OPs)",
    "⚙️ Roteiro e Tempos",
...
with tab_painel:
    ...
    with col_kpi1:
        ...
        <div class='card-metric-critical'>
            <h5 style='margin:0; color:#B91C1C;'>🚨 GARGALOS CRÍTICOS</h5>
            <h2 style='margin:5px 0 0 0; color:#7F1D1D;'>{gargalos_criticos} Recursos</h2>
            <small style='color:#7F1D1D;'>Ocupação diária acima de 100%</small>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_kpi2:
        ...
        <div class='card-metric-warning'>
            <h5 style='margin:0; color:#B45309;'>⚠️ RECURSOS EM ATENÇÃO</h5>
            <h2 style='margin:5px 0 0 0; color:#78350F;'>{dias_atencao} Recursos</h2>
            <small style='color:#78350F;'>Ocupação entre 80% e 100%</small>
        </div>
        """, unsafe_allow_html=True)
        
    with col_kpi3:
        st.markdown(f"""
        <div class='card-metric-normal'>
            <h5 style='margin:0; color:#0369A1;'>⏰ TOTAL HORAS EXTRAS</h5>
            <h2 style='margin:5px 0 0 0; color:#0C4A6E;'>{total_horas_extras_periodo:.1f} Horas</h2>
            <small style='color:#0C4A6E;'>Programadas no período ativo</small>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_kpi4:
        st.markdown(f"""
        <div class='card-metric-normal'>
            <h5 style='margin:0; color:#0369A1;'>🎯 ON-TIME DELIVERY (OTD)</h5>
            <h2 style='margin:5px 0 0 0; color:#0C4A6E;'>{otd_label}</h2>
            <small style='color:#0C4A6E;'>{otd_sub}</small>
        </div>
        """, unsafe_allow_html=True)
