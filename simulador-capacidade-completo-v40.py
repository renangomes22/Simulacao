...
# =====================================================================
st.set_page_config(
    page_title="GCP-Molas | Gêmeo Digital, APS & DDMRP V40",
    page_icon="⚙️",
    layout="wide",
...
if "DESCRIÇÃO DO PRODUTO" not in st.session_state.demandas.columns and "DESCRICAO DO PRODUTO" not in st.session_state.demandas.columns:
    st.session_state.demandas["DESCRIÇÃO DO PRODUTO"] = "-"
if "SEQUENCIA ATUAL" not in st.session_state.demandas.columns and "SEQUENCIA_ATUAL" not in st.session_state.demandas.columns:
    st.session_state.demandas["SEQUENCIA ATUAL"] = "" 
...
with st.sidebar.expander("📤 6. Upload de Planilha de Demanda", expanded=False):
    st.markdown("Importe as planilhas de demanda contendo os campos: **PEDIDO DE VENDA, ORDEM, PRODUTO, DESCRIÇÃO DO PRODUTO, CLIENTE, QUANTIDADE, DATA DE ENTREGA, SEQUENCIA ATUAL**")
    ...
    modelo_dem_csv = "PEDIDO DE VENDA,ORDEM,PRODUTO,DESCRIÇÃO DO PRODUTO,CLIENTE,QUANTIDADE,DATA DE ENTREGA,SEQUENCIA ATUAL\nPV-1010,0105294-002,284269,Mola Helicoidal Dupla,MRS Logística S/A,130,2026-05-06,10\nPV-1011,0106147-002,282001,Mola Tensão Interna,Ferrovia Norte Sul S/A,180,2026-05-05,20"
    st.download_button(
    ...
    if arquivo_demanda is not None and st.session_state.ultimo_arquivo_demanda != arquivo_demanda.name:
        try:
            ...
            if colunas_requeridas_dem.issubset(set(df_up_dem.columns)):
                # Normalizar aliases de colunas adicionais
                if "DESCRICAO DO PRODUTO" in df_up_dem.columns and "DESCRIÇÃO DO PRODUTO" not in df_up_dem.columns:
                    df_up_dem["DESCRIÇÃO DO PRODUTO"] = df_up_dem["DESCRICAO DO PRODUTO"]
                elif "DESCRICAO" in df_up_dem.columns and "DESCRIÇÃO DO PRODUTO" not in df_up_dem.columns:
                    df_up_dem["DESCRIÇÃO DO PRODUTO"] = df_up_dem["DESCRICAO"]
                elif "DESCRIÇÃO DO PRODUTO" not in df_up_dem.columns:
                    df_up_dem["DESCRIÇÃO DO PRODUTO"] = "-"

                if "SEQUENCIA_ATUAL" in df_up_dem.columns and "SEQUENCIA ATUAL" not in df_up_dem.columns:
                    df_up_dem["SEQUENCIA ATUAL"] = df_up_dem["SEQUENCIA_ATUAL"]
                elif "SEQUENCIA" in df_up_dem.columns and "SEQUENCIA ATUAL" not in df_up_dem.columns:
                    df_up_dem["SEQUENCIA ATUAL"] = df_up_dem["SEQUENCIA"]
                elif "SEQUENCIA ATUAL" not in df_up_dem.columns:
                    df_up_dem["SEQUENCIA ATUAL"] = ""
    ...
    with st.spinner("⚡ Executando motor de planejamento de capacidade finita (FCS) e Monte Carlo..."):
        ...
        if st.session_state.simulacao_disparada and not st.session_state.demandas.empty:
            for _, d_row in st.session_state.demandas.iterrows():
                ...
                desc_prod = str(d_row.get("DESCRIÇÃO DO PRODUTO", d_row.get("DESCRICAO DO PRODUTO", "-"))).strip()
                cliente = d_row.get("CLIENTE", "Cliente Geral")
                qtd = float(d_row.get("QUANTIDADE", 0))
                dt_ent = d_row.get("DATA DE ENTREGA")
                seq_atual_raw = d_row.get("SEQUENCIA ATUAL", d_row.get("SEQUENCIA_ATUAL", ""))
                ...
                # Buscar roteiros do produto
                rot_match = st.session_state.roteiros_tempos[st.session_state.roteiros_tempos["Produto"] == prod].copy()
                if rot_match.empty:
                    erros_roteiro.append(prod)
                    continue

                # Filtrar roteiro a partir da SEQUENCIA ATUAL se informada
                seq_start = None
                if pd.notna(seq_atual_raw):
                    s_str = str(seq_atual_raw).strip()
                    if s_str not in ["", "-", "None", "nan"]:
                        try:
                            seq_start = float(s_str)
                        except ValueError:
                            seq_start = None

                if seq_start is not None and seq_start > 0:
                    rot_match = rot_match[rot_match["Sequencia"] >= seq_start]

                if rot_match.empty:
                    # Se após filtrar a sequência atual não sobrou nenhuma operação para o produto
                    continue
                ...
                demandas_ativas_brutas.append({
                    ...
                    "Sequencia_Atual": str(int(seq_start)) if (seq_start is not None and seq_start > 0 and seq_start == int(seq_start)) else (str(seq_start) if seq_start is not None else "-"),
                    "Cliente": cliente,
                    "Quantidade": int(qtd),
        ...
        for op_item in demandas_ativas_brutas:
            ...
            demandas_ativas.append({
                ...
                "Descricao_Produto": op_item.get("Descricao_Produto", "-"),
        ...
        if tipo_planejamento == "Finito (FCS - Finite Capacity Scheduling)" and demandas_ativas:
                    ...
                    for idx, operacao in enumerate(operacoes_da_seq):
                        ...
                        if scheduled_on_days:
                            ...
                            gantt_records.append({
                                ...
                                "Cliente": cliente,
                                "Produto": prod,
                                "Descrição do Produto": op_item.get("Descricao_Produto", "-"),
            ...
            for c in cargas_finitas_detalhadas:
                ...
                gantt_table_records.append({
                    ...
                    "Cliente": c["Cliente"],
                    "Produto": c["Produto"],
                    "Descrição do Produto": c.get("Descricao_Produto", "-"),
...
st.markdown("<div class='main-title'>GCP-Molas - Gêmeo Digital, APS & DDMRP V40</div>", unsafe_allow_html=True)
    ...
    if tipo_planejamento == "Infinito (MRP II / CRP)":
        ...
        if gantt_records:
            ...
            fig_gantt = px.timeline(
                ...
                hover_data=["OP", "Cliente", "Produto", "Descrição do Produto", "Tarefa"],
            ...
            cols_gantt_show = [col for col in ["OP", "Cliente", "Produto", "Descrição do Produto", "Quantidade", "Maquina", "Tarefa", "Início", "Término", "Status"] if col in df_gantt_filtrado.columns]
            st.dataframe(df_gantt_filtrado[cols_gantt_show], use_container_width=True)
        else:
...
# ----------------- TAB 4: DEMANDAS ATIVAS NO PLANEJAMENTO -----------------
    ...
    with col_d2:
        ...
        manual_desc = st.text_input("Descrição do Produto", value="Mola Helicoidal Dupla")
    with col_d3:
        manual_cli = st.text_input("Cliente", value="MRS Logística S/A")
        manual_seq_atual = st.text_input("Sequência Atual (Opcional - ex: 10, 20)", value="")
    with col_d4:
    ...
    if st.button("🚀 Confirmar Lançamento de Demanda", use_container_width=True):
        nova_row = pd.DataFrame([{
            ...
            "ORDEM": manual_op,
            "PRODUTO": manual_prod,
            "DESCRIÇÃO DO PRODUTO": manual_desc,
            "CLIENTE": manual_cli,
            "QUANTIDADE": manual_qtd,
            "DATA DE ENTREGA": manual_data.strftime("%Y-%m-%d"),
            "SEQUENCIA ATUAL": manual_seq_atual.strip()
        }])
    ...
    if st.button("🗑️ Limpar Banco de Dados de Demandas"):
        st.session_state.demandas = pd.DataFrame(columns=[
            "PEDIDO DE VENDA", "ORDEM", "PRODUTO", "DESCRIÇÃO DO PRODUTO", 
            "CLIENTE", "QUANTIDADE", "DATA DE ENTREGA", "SEQUENCIA ATUAL"
        ])
        st.session_state.simulacao_disparada = False # Resetar
